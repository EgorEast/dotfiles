...
- To make some scripts work, you need to modify them from the release a little bit:
  ...
  - **(Optional)** By default, the `mpv-gif.lua` script saves GIFs to `~/mpv/gifs`. To modify this, open `gif.conf`, which is located in `portable_config/script-opts` folder, with a text editor and specify the `dir`, which is output directory for GIFs, as you please. For example `dir="/home/USERNAME/Pictures/mpv-gifs"`.
