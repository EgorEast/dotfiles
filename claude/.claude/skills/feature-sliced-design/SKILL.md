...
- **To Shared**: When you need the same _infrastructure_ in multiple places (modal manager, date formatter, UI components)
- **To Entities**: When you have a clear _business domain model_ that's used across multiple features
- **To Features**: When you have a complete _user interaction_ that's reused in multiple places
...
export const fetchUser = (id: string) => apiClient.get(`/users/${id}`);
